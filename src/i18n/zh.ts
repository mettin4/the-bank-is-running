...
export const zh: Dict = {
  ...
  'rail.epochStateHint':
    '手续费流向取决于当前 EPOCH 净流量的正负。净流量为正时买入硬储备，为负时回购并销毁代币。',
  ...
  'nar.raised': '资金正在流入。银行挣得一次加息，手续费用于买入硬储备。',
  'nar.cutStands': '流入已恢复，但此前的降息仍未撤销。加息必须逐个 EPOCH 挣得。',
  ...
  'stat.burnedHint': '被许可、回购、退出费与吊销永久销毁的代币。每一次销毁都会拉低硬顶。',
...
export const zhEvents: EventDict = {
  ...
  charterDissolved: ({ id, amount }) =>
    `牌照 #${pad4(id)} 注销 · 最后一个分行退役 · 提取 ${int(amount)}`,
  charterRetired: ({ id, branches, amount, feeRate }) =>
    `牌照 #${pad4(id)} 退役 ${branches} 个分行 · 提取 ${int(amount)} · 退出费 ${pct(feeRate, 2)}`,
  ...
  reserveAdded: ({ oz, eth }) => `硬储备 +${dec(oz, 2)} OZ · 耗用 ${dec(eth, 2)} ETH`,
  ...
  resolutionFeeHigh: ({ feeRate }) =>
    `退出费突破 10% · 门口 ${pct(feeRate, 2)} · 一半销毁，一半付给留守者`,
};
...
export const zhAssumed: AssumedDict = {
  ...
  LICENSE_SETTLEMENT: {
    label: '许可结算',
    note: '许可费以 $STANDARD 支付并销毁，但代币只有在提取后才存在。因此累积的账本余额在同一笔交易内铸造并销毁，这就是累计铸造与累计销毁同步上升而流通量不变的原因。',
  },
  DORMANT_AGENTS: {
    label: '休眠银行家',
